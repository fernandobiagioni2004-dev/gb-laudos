  ...
  const radReport = useMemo(() => {
    return radiologists.map(r => {
      const rExams = exams.filter(e => {
        const okRad = radFilter === 'all' || e.radiologistId === radFilter;
        const okClient = clientFilter === 'all' || e.clientId === clientFilter;
        const okType = examTypeFilter === 'all' || e.examTypeId === examTypeFilter;
        return e.radiologistId === r.id && e.status === 'Finalizado' && okRad && okClient && okType;
      });
      const qty = rExams.length;
      const total = rExams.reduce((a, e) => a + e.radiologistValue, 0);
      const unit = qty > 0 ? total / qty : 0;
      return { ...r, qty, unit, total };
    }).filter(r => r.qty > 0);
  }, [exams, radFilter, clientFilter, examTypeFilter]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Análise financeira e operacional</p>
      </div>
        ...
        {(['radiologista', 'cliente'] as const).map(tab => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all border ${
              activeTab === tab ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground'
            }`}
          >
        ...
        <div className="space-y-4">
          <div className="flex gap-3 flex-wrap items-end">
            <div className="space-y-1">
              <Label className="text-xs">Radiologista</Label>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-muted/30">
                        {['Radiologista', 'Software', 'Qtde', 'Vlr. Unitário', 'Total'].map(h => (
                    ...
                    <tbody>
                      {radReport.map(r => (
                        <tr key={r.id} className="border-b border-border/50 hover:bg-muted/20">
                          ...
                          <td className="py-3 px-4 text-muted-foreground">{r.software.join(', ')}</td>
              ...
              <Card key={c.id}>
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-emerald-400">{fmt(c.revenue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Pago</span>
                    <span className="text-amber-400">{fmt(c.paid)}</span>
                  </div>
                  <div className="flex justify-between border-t border-border pt-2">
                    <span className="font-medium">Margem</span>
                    <span className="font-bold text-blue-400">{fmt(c.margin)}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
